...
def main():
    ...
    search_parser.add_argument("--page", "-p", type=int,
                               help="Page number to search (0-based, omit to search all pages)")
    ...
    search_parser.add_argument("--array-format", "-af", action="store_true",
                               help="Use array format for JSON output instead of hierarchical format")
    search_parser.add_argument("--group-by-text", "-gbt", action="store_true", default=True,
                               help="Group results by text in array format output")
    ...
    parse_parser.add_argument("--with-coordinates", "-c", action="store_true",
                              help="Include text coordinates in output")
    parse_parser.add_argument("--array-format", "-af", action="store_true", default=True,
                              help="Use array format for JSON output instead of hierarchical format")
    parse_parser.add_argument("--group-by-text", "-gbt", action="store_true", default=True,
                              help="Group results by text in array format output")
                ...
                if args.json:
                    if args.array_format:
                        ...
                        if args.json_file:
                            ...
                            os.makedirs(os.path.dirname(json_file_path) if os.path.dirname(json_file_path) else '.',
                                        exist_ok=True)
                        else:
                            base_name = os.path.basename(args.input)
                    ...
                    for i, result in enumerate(results):
                        ...
                        if "rect" in result:
                            rect = result["rect"]
                            print(
                                f"     Position: x0={rect['x0']:.2f}, y0={rect['y0']:.2f}, x1={rect['x1']:.2f}, y1={rect['y1']:.2f}")
            else:
